...
export const addVolunteer: RequestHandler = async (req, res, next) => {
    ...
    try {
        ...
        const requiredProperties = ['first_name', 'last_name', 'email', 'phone', 'signed_up_for_updates'];

        for (const prop of requiredProperties) {
            if (!volunteerData.hasOwnProperty(prop)) {
                return res.status(400).json({ error: `Missing property: ${prop}. Please provide all required fields.` });
            }
        }
