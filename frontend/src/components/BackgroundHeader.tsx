...
const BackgroundHeader = ({
  ...
  return (
    <div className="w-full h-screen relative">
      ...
      <div className="flex justify-center items-center space-x-2 absolute bottom-5 left-1/2 transform -translate-x-1/2" style={{ gap: '16px' }}>
      <button
  onClick={() => { setActiveIndex((prevIndex) => (prevIndex === 0 ? backgroundImageURIs.length - 1 : prevIndex - 1)); }}
...
<button
  onClick={() => { setActiveIndex((prevIndex) => (prevIndex === backgroundImageURIs.length - 1 ? 0 : prevIndex + 1)); }}
  className={`text-white p-2 ${styles.arrowButton}`}
