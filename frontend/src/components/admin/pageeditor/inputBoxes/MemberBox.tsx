...
import { Member, updateMember } from "../../../../api/member";
...
type MemberBoxProps = {
  idx: number;
  member: Member;
  handleEditMember: (m: Member, isTextEdit: boolean) => void;
  handleRemoveMember: (m: Member) => void;
};
...
export default function MemberBox({
  ...
  const handleChangeName = (e: ChangeEvent<HTMLTextAreaElement>) => {
    ...
    handleEditMember(updatedMember, true);
    setMemberState(updatedMember);
  };

  const handleChangeRole = (e: ChangeEvent<HTMLTextAreaElement>) => {
    const updatedMember = {
      ...member,
      role: e.target.value,
    };
  ...
  const handleSetUrl = (_url: string) => {
    ...
    handleEditMember(updatedMember, false);
    updateMember(updatedMember).catch(console.error);
    setMemberState(updatedMember);
  };

  const handleDelete = () => {
    // if url exists and is not "" delete it
    if (memberState.profilePictureURL && memberState.profilePictureURL !== "") {
      deleteFile(memberState.profilePictureURL).catch(console.error);
    }
    handleRemoveMember(memberState);
  };
    ...
    <div className="group flex flex-row w-full h-auto mt-4">
      ...
      <div className="flex flex-row flex-wrap justify-between w-full">
        <div className="w-[469px] mb-2">
          <p className="mb-[2px] text-base font-normal font-['Open Sans']">Staff Name</p>
          <TextareaAutosize
            className="flex flex-row items-center w-[469px] h-[56px] px-3 py-2 bg-[#e9eef7] rounded outline outline-1 outline-[#d8d8d8] resize-none"
            onChange={handleChangeName}
            value={member.name}
          />
        </div>
        <div className="w-[469px] mb-2">
          <p className="mb-[2px] text-base font-normal font-['Open Sans']">Staff Position</p>
          <TextareaAutosize
            className="flex flex-row items-center w-[469px] h-[56px] px-3 py-2 bg-[#e9eef7] rounded outline outline-1 outline-[#d8d8d8] resize-none"
